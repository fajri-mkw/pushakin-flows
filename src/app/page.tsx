...
import { Wrench, Clock, AlertTriangle, Shield, Settings } from 'lucide-react'
...
function MaintenanceView({ message, onAdminLogin }: { message: string | null; onAdminLogin: () => void }) {
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      if (!response.ok) { setError(data.error || 'Login gagal'); setIsLoading(false); return }
      if (data.user?.role !== 'Admin') { setError('Hanya Admin yang dapat mengakses saat maintenance'); setIsLoading(false); return }
      localStorage.setItem('adminMaintenanceAccess', 'true')
      onAdminLogin()
    } catch { setError('Terjadi kesalahan saat login') }
      ...
      <div className="max-w-md w-full">
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center border border-white/20 shadow-2xl">
          ...
          <div className="bg-white/5 rounded-xl p-4 mb-6 text-left space-y-3">
            <div className="flex items-center gap-3 text-white/80"><Clock className="w-5 h-5 text-amber-400" /><span className="text-sm">Estimasi: Beberapa menit</span></div>
            <div className="flex items-center gap-3 text-white/80"><AlertTriangle className="w-5 h-5 text-amber-400" /><span className="text-sm">Silakan coba beberapa saat lagi</span></div>
          </div>
          <button onClick={() => setShowAdmin(!showAdmin)} className="text-white/40 hover:text-white/60 text-xs flex items-center gap-1 mx-auto mb-4"><Settings className="w-3 h-3" /><span>Akses Admin</span></button>
          {showAdmin && (
            <div className="bg-white/5 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-center gap-2 text-violet-300 text-sm mb-2"><Shield className="w-4 h-4" /><span>Login Admin</span></div>
              ...
              <button onClick={handleAdminLogin} disabled={isLoading || !adminEmail || !adminPassword} className="w-full py-2 bg-violet-600 hover:bg-violet-700 disabled:bg-violet-600/50 text-white rounded-lg text-sm font-medium transition-colors">{isLoading ? 'Memproses...' : 'Login Admin'}</button>
            </div>
          )}
        </div>
...
function AppContent() {
  const { currentUser, activeView, setUsers, setProjects, setNotifications } = useAppStore()
  ...
  const [maintenanceData, setMaintenanceData] = useState<{ maintenance: boolean; message: string | null } | null>(null)
  const [adminMaintenanceAccess, setAdminMaintenanceAccess] = useState(false)

  useEffect(() => { setAdminMaintenanceAccess(localStorage.getItem('adminMaintenanceAccess') === 'true') }, [])
  useEffect(() => {
    const checkMaintenance = async () => {
      try {
        const res = await fetch('/api/maintenance')
        if (res.ok) { const data = await res.json(); setMaintenanceData({ maintenance: data.maintenance ?? false, message: data.message ?? null }) }
        else setMaintenanceData({ maintenance: false, message: null })
      } catch { setMaintenanceData({ maintenance: false, message: null }) }
    }
    checkMaintenance()
  }, [])
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (projectsRes.ok) { const projectsData = await projectsRes.json(); setProjects(projectsData) }
        setIsLoading(false)
  ...
  const handleSeed = async () => {
    ...
    try {
      const res = await fetch('/api/seed'); const data = await res.json()
      if (!res.ok || !data.success) { setSeedError(data.details || data.error || 'Terjadi kesalahan'); return }
      const usersRes = await fetch('/api/users')
      if (usersRes.ok) { const usersData = await usersRes.json(); setUsersState(usersData); setUsers(usersData) }
  ...
  if (maintenanceData.maintenance && currentUser?.role !== 'Admin' && !adminMaintenanceAccess) return <MaintenanceView message={maintenanceData.message} onAdminLogin={handleAdminLogin} />
  if (activeView === 'login' || !currentUser) return <><LoginView onSeed={handleSeed} isSeeding={isSeeding} seedError={seedError} /><DialogModal /></>
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-violet-50 font-sans selection:bg-violet-200 overflow-hidden">
      {maintenanceData.maintenance && (currentUser?.role === 'Admin' || adminMaintenanceAccess) && (
        <div className="fixed top-0 left-0 right-0 bg-amber-500 text-amber-900 text-center py-2 text-sm font-medium z-50 flex items-center justify-center gap-2"><Wrench className="w-4 h-4" /><span>Mode Maintenance Aktif - Hanya Admin yang dapat mengakses</span></div>
      )}
      <Sidebar />
